...
async def osint_db_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ต่อยอด: ทำดัชนีระเบียนที่เพิ่งยืนยันลง Elasticsearch (best-effort) เพื่อค้น
    # ย้อนหลังผ่าน /dbsearch — ถ้าไม่ได้ตั้งค่า ES ไว้ก็ข้ามเงียบ ไม่กระทบการบันทึก
    es_note = ""
    if not result.get("duplicate") and osint_es.es_configured():
        try:
            if await asyncio.to_thread(osint_es.index_finding, record):
                es_note = "\n🔎 ทำดัชนีลง Elasticsearch แล้ว (ค้นย้อนหลังด้วย /dbsearch)"
        except Exception:
            logger.exception("OSINT ES INDEX ERROR id=%s", record.get("id"))
...
async def cmd_dbsearch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ค้นย้อนหลังใน "ฐานข้อมูลผลค้น OSINT ที่ยืนยันแล้ว" (osint_db)

    ต่อยอดการค้นหาของ OSINT ด้วย Elasticsearch: ถ้าตั้งค่า ES ไว้และต่อได้ จะค้นผ่าน
    ES (full-text + fuzzy) ถ้าไม่ ก็ fallback ไปค้นในไฟล์ JSON ตรง ๆ — ไม่ดึงข้อมูล
    ใหม่จากที่ไหน ค้นเฉพาะสิ่งที่ Admin เคยกดยืนยันบันทึกไว้แล้วเท่านั้น (Admin only)"""
    if not await is_admin(update, context):
        return await update.message.reply_text("❌ ค้นฐานข้อมูล OSINT ได้เฉพาะ Admin")

    query = " ".join(context.args).strip()
    if not query:
        return await update.message.reply_text(
            "ใช้งาน: /dbsearch <คำค้น | อีเมล | โดเมน | โฮสต์ | รหัสระเบียน>\n"
            "ค้นย้อนหลังในผลที่เคยกดยืนยันบันทึกไว้ (ต่อยอดจาก /search)\n"
            "ตัวอย่าง: /dbsearch acme.co.th")

    limit = SEARCH_RESULTS_DISPLAY_CAP
    hits = None
    via = ""
    # (1) ลองค้นผ่าน Elasticsearch ก่อน (ถ้าตั้งค่าและต่อได้)
    if osint_es.es_configured():
        try:
            hits = await asyncio.to_thread(osint_es.search, query, limit)
        except Exception:
            logger.exception("OSINT ES SEARCH ERROR")
            hits = None
        if hits is not None:
            via = "Elasticsearch"

    # (2) ไม่มี ES / ค้น ES ไม่ได้ -> fallback ค้นในไฟล์ JSON ตรง ๆ
    if hits is None:
        db = await asyncio.to_thread(osint_db.load_db)
        hits = osint_es.local_search(db.get("entries", []), query, limit)
        via = "ไฟล์ JSON"
...
async def cmd_bbprogram(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is enforced inside scope_policy.set_program_status(), not here --
    app.py only parses input, validates basic syntax, and calls the
    policy API, per the Phase 5 app.py/scope_policy.py split."""
    args = context.args or []
    if not args:
        return await update.message.reply_text(
            "ใช้งาน:\n"
            "/bbprogram new <ชื่อ...>\n"
            "/bbprogram active <program_id>\n"
            "/bbprogram pause <program_id>\n"
            "/bbprogram archive <program_id>\n"
            "/bbprogram list"
        )
    sub = args[0].lower()
    user = update.effective_user
    ...
    if sub == "new":
        if not await is_admin(update, context):
            return await update.message.reply_text("❌ คำสั่งนี้ใช้ได้เฉพาะ Admin")
        if len(args) < 2:
            return await update.message.reply_text("ใช้งาน: /bbprogram new <ชื่อ...>")
        name = " ".join(args[1:])
        program_id = create_program(chat_id, name, created_by=user.id)
        await update.message.reply_text(
            f"✅ สร้าง Program #{program_id} แล้ว (สถานะ PAUSED)\n"
            f"ใช้ /bbprogram active {program_id} เพื่อเปิดใช้งาน"
        )

    elif sub in ("active", "pause", "archive"):
        if not await is_admin(update, context):
            return await update.message.reply_text("❌ คำสั่งนี้ใช้ได้เฉพาะ Admin")
        ...
        target_status = {
            ...
            "pause": ProgramStatus.PAUSED.value,
            "archive": ProgramStatus.ARCHIVED.value,
        }[sub]
        ok = set_program_status(program_id, target_status, user.id)
        if not ok:
            return await update.message.reply_text(
                f"❌ เปลี่ยนสถานะไม่ได้ ({program['status']} -> {target_status} ไม่ใช่ transition ที่อนุญาต)"
            )
        await update.message.reply_text(f"✅ Program #{program_id}: {program['status']} -> {target_status}")
...
async def cmd_bbauth(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/bbauth import|review|revoke|list -- Authorization Artifact
    workflow. import always creates PENDING_REVIEW; only a separate,
    explicitly logged review_authorization() call (admin-only, below)
    can move it to ACTIVE/REJECTED. Being an admin never substitutes
    for this -- scope_policy.evaluate_target() has no is_admin input."""
    args = context.args or []
    if not args:
        return await update.message.reply_text(
            "ใช้งาน:\n"
            "/bbauth import <program_id> <source_type> [อ้างอิง...]\n"
            "/bbauth review <authorization_id> approve|reject [หมายเหตุ...]\n"
    ...
    if sub == "import":
        ...
        if len(args) < 3 or not args[1].isdigit():
            return await update.message.reply_text(
                "ใช้งาน: /bbauth import <program_id> <source_type> [อ้างอิง...]"
            )
        program_id = int(args[1])
        source_type = args[2]
        source_reference = " ".join(args[3:])
        authorization_id = import_authorization(
            program_id, source_type, actor_user_id=user.id, source_reference=source_reference
        )
        if authorization_id is None:
            return await update.message.reply_text("❌ ไม่พบ Program นี้")
        await update.message.reply_text(
            f"✅ บันทึก Authorization #{authorization_id} แล้ว (สถานะ PENDING_REVIEW)\n"
            f"ใช้ /bbauth review {authorization_id} approve เพื่อ review"
        )
        ...
        if len(args) < 3 or not args[1].isdigit() or args[2].lower() not in ("approve", "reject"):
            return await update.message.reply_text(
                "ใช้งาน: /bbauth review <authorization_id> approve|reject [หมายเหตุ...]"
            )
        authorization_id = int(args[1])
        approve = args[2].lower() == "approve"
        notes = " ".join(args[3:])
        ok = review_authorization(authorization_id, approve, reviewer_user_id=user.id, notes=notes)
        if not ok:
            return await update.message.reply_text(
                "❌ Review ไม่สำเร็จ (ไม่พบ Authorization นี้ หรือสถานะไม่ใช่ PENDING_REVIEW)"
            )
        await update.message.reply_text(
            f"✅ Authorization #{authorization_id} -> {'ACTIVE' if approve else 'REJECTED'}"
        )
        ...
        authorization_id = int(args[1])
        ok = revoke_authorization(authorization_id, actor_user_id=user.id)
        if not ok:
            return await update.message.reply_text(
                "❌ Revoke ไม่สำเร็จ (ไม่พบ Authorization นี้ หรือสถานะถูก revoke/reject/expire ไปแล้ว)"
            )
        await update.message.reply_text(f"✅ Authorization #{authorization_id} -> REVOKED")

    elif sub == "list":
        if len(args) < 2 or not args[1].isdigit():
            return await update.message.reply_text("ใช้งาน: /bbauth list <program_id>")
        authorizations = list_authorizations(int(args[1]))
...
async def cmd_bbscope(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/bbscope add|remove|list -- Scope Rule management. Only ever
    calls scope_policy.add_scope_rule/remove_scope_rule/list_scope_rules
    -- no domain/IP/CIDR matching logic lives in app.py; that stays
    solely inside scope_policy.py's evaluate_target()/_rule_matches()."""
    args = context.args or []
    if not args:
        return await update.message.reply_text(
            "ใช้งาน:\n"
    ...
    if sub == "add":
        ...
        pattern = args[4]
        rule_id = add_scope_rule(program_id, rule_type, target_type, pattern, actor_user_id=user.id)
        if rule_id is None:
            return await update.message.reply_text(
                "❌ เพิ่ม Scope Rule ไม่สำเร็จ (ไม่พบ Program, rule_type/target_type ไม่ถูกต้อง, "
                "หรือ pattern ไม่ถูกต้อง)"
            )
        await update.message.reply_text(f"✅ เพิ่ม Scope Rule #{rule_id}: {rule_type} {target_type} {pattern}")
        ...
        rule_id = int(args[1])
        ok = remove_scope_rule(rule_id, actor_user_id=user.id)
        if not ok:
            return await update.message.reply_text("❌ ไม่พบ Scope Rule นี้")
        await update.message.reply_text(f"✅ ลบ Scope Rule #{rule_id} แล้ว")

    elif sub == "list":
        if len(args) < 2 or not args[1].isdigit():
            return await update.message.reply_text("ใช้งาน: /bbscope list <program_id>")
        rules = list_scope_rules(int(args[1]))
        if not rules:
...
async def cmd_bbcheck(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/bbcheck <program_id> <target> -- read-only scope-policy query,
    open to any chat member (no mutation happens here, so no is_admin
    gate). Calls the exact same evaluate_target() gate create_finding()
    uses, so a target can be checked before attempting /bbfinding new.
    Reuses _deny_text_finding()'s Thai reason mapping for DENY text --
    the Reason space evaluate_target() returns is identical either way,
    so this does not duplicate that mapping."""
    args = context.args or []
...
_DEBT_CB_DASHBOARD = "dbt:dash"
_DEBT_CB_SELECT = "dbt:sel"     # dbt:sel:<ref>:<page>
_DEBT_CB_CLOSE_ASK = "dbt:ca"   # dbt:ca:<ref>
_DEBT_CB_CLOSE_DO = "dbt:cd"    # dbt:cd:<ref>
DEBT_DETAIL_PAGE_SIZE = 10     # Requirement #14

async def _build_debt_dashboard(chat_id: int):
    """สร้าง (text, InlineKeyboardMarkup) ของ /debt เปล่า -- ปุ่ม 1 อันต่อคนที่
    ยังค้างชำระ กดแล้วเปิดการ์ด accordion ของคนนั้น"""
    summary = dl.summarize_debtors(chat_id, status=dl.EntryStatus.UNPAID.value)
    text = dr.format_debt_dashboard(summary)
    buttons = [
        [InlineKeyboardButton(
            dr.format_debtor_button_label(d["debtor_name"], d["total_satang"]),
            callback_data=f"{_DEBT_CB_SELECT}:{dl.debtor_ref(chat_id, d['debtor_name'])}:1",
        )]
...
async def _build_debtor_detail_view(chat_id: int, debtor_name: str, page: int = 1):
    entries = dl.list_entries(
        chat_id, status=dl.EntryStatus.UNPAID.value,
    ...
    nav_row = []
    if page > 1:
        nav_row.append(InlineKeyboardButton("◀️ ก่อนหน้า", callback_data=f"{_DEBT_CB_SELECT}:{ref}:{page-1}"))
    if page < total_pages:
        nav_row.append(InlineKeyboardButton("ถัดไป ▶️", callback_data=f"{_DEBT_CB_SELECT}:{ref}:{page+1}"))

    rows = [nav_row] if nav_row else []
    if entries:
        rows.append([InlineKeyboardButton("✅ ปิดยอดทั้งหมด (Admin)", callback_data=f"{_DEBT_CB_CLOSE_ASK}:{ref}")])
    rows.append([InlineKeyboardButton("◀️ กลับไปหน้ารวม", callback_data=_DEBT_CB_DASHBOARD)])
    return text, InlineKeyboardMarkup(rows)


async def on_debt_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Router เดียวสำหรับปุ่มทั้งหมดของ /debt dashboard --
    resolve_debtor_ref คืน None แบบ safe เมื่อ ref เก่า/คนถูกปิดยอดไปแล้ว
    ไม่ทำให้ bot crash"""
    query = update.callback_query
    chat_id = update.effective_chat.id
    data = query.data or ""
    ...
    if action in ("ca", "cd") and not await is_admin(update, context):
        return await query.answer("❌ คำสั่งนี้ใช้ได้เฉพาะ Admin", show_alert=True)
    ...
    try:
        if action == "sel":
            ...
            if name is None:
                text, markup = await _build_debt_dashboard(chat_id)
                return await query.edit_message_text(
                    "⚠️ ไม่พบรายการนี้แล้ว (อาจถูกปิดยอดไปแล้ว)\n\n" + text, reply_markup=markup
                )
            text, markup = await _build_debtor_detail_view(chat_id, name, page=page)
            return await query.edit_message_text(text, reply_markup=markup)
            ...
            if name is None:
                text, markup = await _build_debt_dashboard(chat_id)
                return await query.edit_message_text("⚠️ ไม่พบรายการนี้แล้ว\n\n" + text, reply_markup=markup)
            confirm_markup = InlineKeyboardMarkup([
                [InlineKeyboardButton(f"✅ ยืนยันปิดยอด {name}", callback_data=f"{_DEBT_CB_CLOSE_DO}:{ref}")],
                [InlineKeyboardButton("◀️ ยกเลิก", callback_data=f"{_DEBT_CB_SELECT}:{ref}:1")],
            ])
            return await query.edit_message_text(
                f"⚠️ ยืนยันปิดยอดค้างชำระทั้งหมดของ {name}?", reply_markup=confirm_markup
            )
            ...
            if name is None:
                text, markup = await _build_debt_dashboard(chat_id)
                return await query.edit_message_text("⚠️ ไม่พบรายการนี้แล้ว\n\n" + text, reply_markup=markup)
            result = dl.mark_debtor_paid(chat_id, name, actor_user_id=update.effective_user.id)
            text, markup = await _build_debt_dashboard(chat_id)
            if not result.ok:
                return await query.edit_message_text(_deny_text_debt(result.reason) + "\n\n" + text, reply_markup=markup)
            return await query.edit_message_text(
